...
TestCase {
  ...
  Component {
    id: openerComponent
    Plugin.SetupOpener { timeoutMs: 300; settleMs: 1 }
  }
  ...
  function argvRecipe(argv) {
    return { type: "argv", argv: argv }
  }

  function tiledWindow(recipe, x, y, width, height) {
    return { recipe: recipe, floating: false, rect: { x: x, y: y, width: width, height: height } }
  }

  function floatingWindow(recipe, x, y, width, height) {
    return { recipe: recipe, floating: true, rect: { x: x, y: y, width: width, height: height } }
  }
  ...
  function test_launchesAnArgvRecipeThroughExecDetached() {
    ...
    opener.open({ windows: [tiledWindow(argvRecipe(["some-tool", "--flag"]), 0, 0, 1, 1)] }, { width: 1920, height: 1080 })
  ...
  function test_desktopEntryRecipeResolvesByItsSavedId() {
    ...
    DesktopEntries.testSetEntries({
      "firefox.desktop": { id: "firefox.desktop", execute: function () { calls.push("ran") } }
    })
    var opener = makeOpener()
    opener.open({ windows: [tiledWindow(desktopRecipe("firefox.desktop"), 0, 0, 1, 1)] }, { width: 1920, height: 1080 })
    compare(calls, ["ran"])
  }
  ...
  function test_aMissingDesktopEntryLaunchesNothingButStillTimesOutCleanly() {
    var opener = makeOpener()
    opener.open({ windows: [tiledWindow(desktopRecipe("gone.desktop"), 0, 0, 1, 1)] }, { width: 1920, height: 1080 })
  ...
  function test_aSingleWindowFinishesOnceItAppears() {
    DesktopEntries.testSetEntries({ a: { id: "a", execute: function () {} } })
    var opener = makeOpener()
    opener.open({ windows: [tiledWindow(desktopRecipe("a"), 0, 0, 1, 1)] }, { width: 1920, height: 1080 })
  ...
  function test_focusesAndPreselectsBeforeTheSecondWindow() {
    DesktopEntries.testSetEntries({
      a: { id: "a", execute: function () {} },
      b: { id: "b", execute: function () {} }
    })
    var opener = makeOpener()
    opener.open({
      windows: [
        tiledWindow(desktopRecipe("a"), 0, 0, 0.5, 1),
        tiledWindow(desktopRecipe("b"), 0.5, 0, 0.5, 1)
      ]
  ...
  function test_preselectsDownForAStackedSplit() {
    ...
    opener.open({
      windows: [
        tiledWindow(desktopRecipe("a"), 0, 0, 1, 0.5),
        tiledWindow(desktopRecipe("b"), 0, 0.5, 1, 0.5)
      ]
    }, { width: 1920, height: 1080 })
  ...
  function test_floatingWindowsOpenAfterTheTiledOnes() {
    ...
    opener.open({
      windows: [
        floatingWindow(desktopRecipe("b"), 0.25, 0.25, 0.5, 0.5),
        tiledWindow(desktopRecipe("a"), 0, 0, 1, 1)
      ]
    }, { width: 1920, height: 1080 })
  ...
  function test_anEmptySetupFinishesImmediately() {
    var opener = makeOpener()
    opener.open({ windows: [] }, { width: 1920, height: 1080 })
  ...
  function test_doesNotStartASecondPlanWhileOneIsRunning() {
    var calls = []
    DesktopEntries.testSetEntries({ a: { id: "a", execute: function () { calls.push("a") } } })
    var opener = makeOpener()
    opener.open({ windows: [tiledWindow(desktopRecipe("a"), 0, 0, 1, 1)] }, { width: 1920, height: 1080 })
    opener.open({ windows: [tiledWindow(desktopRecipe("a"), 0, 0, 1, 1)] }, { width: 1920, height: 1080 })
  ...
  function test_aStepThatNeverAppearsIsSkippedAfterTheTimeout() {
    ...
    tryVerify(function () { return finishedSpy.count === 1 }, 2000)
    compare(finishedSpy.signalArguments[0][0], false)
  }
}
